...
Queue.prototype.watchStuckJobs = function( ms, queue ) {
  var client = this.client
    , self   = this
    , ms     = ms || 1000;
  ...
  if (queue) {
    script =
      'local need_fix = 0\n\
       local v = "' + prefix + ':jobs:' + queue + ':inactive"\n\
      ...
      return { need_fix, queue, pending, v, jt }';
  } else {
    script =
      'local cursor = "0"\n\
      local need_fix = 0\n\
  ...
  this.stuck_job_watch = setInterval(function() {
    const stuck_job_start = new Date();
    client.eval(script, 0, function( err, fixes ) {
      if( err ) {
